...
CONTAINS
   ...
   SUBROUTINE icb_stp( kt, Kbb, Kmm )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) :: ts_trnds_icb_tmp
      !
 ...
 9100 FORMAT('kt= ',i8, ' day= ',i8,' secs=',i8)
      ...
      IF( ASSOCIATED(first_berg) )   CALL icb_thm( kt, Kbb, Kmm )       ! Ice berg thermodynamics (melting) + rolling

        ts_trnds_icb_tmp(:,:,:) = ts_trnds_icb(:,:,:,jp_tem)
        CALL lbc_lnk( 'icbstp', ts_trnds_icb_tmp, 'T', 1.0_wp )
        ts_trnds_icb(:,:,:,jp_tem) = ts_trnds_icb_tmp(:,:,:)

        ts_trnds_icb_tmp(:,:,:) = ts_trnds_icb(:,:,:,jp_sal)
        CALL lbc_lnk( 'icbstp', ts_trnds_icb_tmp, 'T', 1.0_wp )
        ts_trnds_icb(:,:,:,jp_sal) = ts_trnds_icb_tmp(:,:,:)
      !
      !
