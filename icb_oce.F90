...
!TOM> no horizontal drag for sea ice! real, PARAMETER :: pp_Cd_ih=0.0012 ! (lateral) Drag coeff. between bergs and sea-ice
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) ::   ts_trnds_icb             !: 3D T-S trends due to bergs [Celsius/s,psu/s]
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:,:) ::   ts_trnds_icb_b           !: 3D T-S trends due to bergs [Celsius/s,psu/s]
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   INTEGER FUNCTION icb_alloc()
      ...
      ALLOCATE( ts_trnds_icb   (jpi,jpj,jpk,jpts)                          ,     &
         &      STAT=ill )
      icb_alloc = icb_alloc + ill

      ALLOCATE( ts_trnds_icb_b   (jpi,jpj,jpk,jpts)                          ,     &
         &      STAT=ill )
      icb_alloc = icb_alloc + ill
